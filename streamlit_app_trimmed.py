...
tabs = ["FlashResponse", "Media", "Ideas", "Media Missinformation"] 
article_file = 'heat-wave/combined_8_uk_articles.txt'
gov_file = article_file
...
if selected_tab == "FlashResponse":
    ...
    st.header("Summaries or Sentiments")
    ...
    media_type = st.selectbox("Select a Type of analysis", ["Summary", "Descriptive Sentiment"])

    if media_type == "Summary":
        query = f'Considering the following {URLS}. Summarise each separte article in 1 sentence.'
    elif media_type == "Descriptive Sentiment":
...
elif selected_tab == "Media Missinformation":
